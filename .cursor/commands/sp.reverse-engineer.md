...
When **`roam_*` tools** are available, you **MUST** use them as the primary graph-backed view of the repo—**`roam_map`**, **`roam_understand`**, **`roam_layers`**, **`roam_endpoints`**—in addition to file reads. If Roam is not configured, fall back to manual exploration only. **Disclosure:** one line `Roam MCP: …` in the final deliverable summary (and PHR when applicable).
