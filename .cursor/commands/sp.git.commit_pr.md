...
When **`roam_*` tools** are available and the change set is large or cross-cutting, you **MAY** call **`roam_diff`** or **`roam_map`** once to improve the commit message or PR description; not required. **Disclosure:** optional `Roam MCP: …` in the final reply if used.
...
1. Resolve **`FEATURE_DIR`** for the feature this branch belongs to (e.g. `specs/$(git rev-parse --abbrev-ref HEAD)` when that path exists).
2. Ensure **`SDD-TIMELINE.md`** exists (create from **`.specify/templates/sdd-timeline-template.md`** if missing).
3. UTC timestamp: `date -u +"%Y-%m-%dT%H:%M:%SZ"`.
4. **Append** `| commit_pr | <timestamp> |`
5. **Final reply:** paste the **full table** under **### SDD execution time** — end-to-end time is **last timestamp minus first** in the table.
