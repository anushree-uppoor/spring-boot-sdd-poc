...
When **`roam_*` tools** are available, you **MUST** call **`roam_layers`** or **`roam_map`** once to align ADR topics with actual architecture boundaries. If unavailable, proceed. **Disclosure:** one line `Roam MCP: …` in the final summary (and PHR when applicable).
