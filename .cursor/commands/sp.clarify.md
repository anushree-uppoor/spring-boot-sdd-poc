...
When **`roam_*` tools** are available, you **MUST** call at least one relevant tool **after** loading the spec and **before** the first clarification question—typically **`roam_map`** or **`roam_understand`** (repo layout), and **`roam_endpoints`** if the spec mentions HTTP/API behavior—to ground questions in the real codebase. If Roam is not configured, proceed. **Disclosure:** one line `Roam MCP: …` in the completion report (and PHR when applicable).
